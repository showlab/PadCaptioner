...
def get_rope_index_25(
    ...
    if input_ids is not None and (
        ...
        for i, input_ids in enumerate(total_input_ids):
            ...
            video_nums = (vision_tokens == video_token_id).sum()    # single start token; the sequence is a plain concat of video and audio tokens
            ...
            for _ in range(video_nums):
                if video_token_id in input_tokens and remain_videos > 0:
                    ed_video = input_tokens.index(video_token_id, st)   # locate the next video token from position st (0 initially)
                else:
                ...
                )   # tokens before the video/audio block
                ...
                t_index = time_tensor_long.flatten()    # Qwen2.5-VL temporal ids, e.g. 0,2,4 at 0.5fps due to the temporal pooling of 2
                ...
                h_index_audio = torch.zeros_like(time_index_audio)  # zeroed for audio
            ...
            if st < len(input_tokens):
                if len(llm_pos_ids_list) > 0:
                    # max over the time dim (dim 0) of the last audio block
                ...
                llm_pos_ids_list.append(
                    torch.arange(text_len).view(1, -1).expand(3, -1) + st_idx
                )   # append the text following the video/audio block
        ...
        for i, input_ids in enumerate(total_input_ids):
            input_ids = input_ids[attention_mask[i] == 1]
            audio_nums = 0
            # locate the audio start (same start token as vision)
            vision_start_indices = torch.argwhere(
                input_ids == vision_start_token_id
            ).squeeze(1)
            vision_tokens = input_ids[vision_start_indices + 1]
            ...
            for _ in range(audio_nums):
                # find the end of the current audio block
                ...
                # audio_lengths is already the number of time-dimension tokens
                llm_grid_t = audio_lengths[audio_index]  # number of time-dimension tokens
                llm_grid_h = 1  # audio has no height dim
                llm_grid_w = 1  # audio has no width dim
            ...
            # handle the remaining text tokens
            if st < len(input_tokens):
                if len(llm_pos_ids_list) > 0:
                    # max over the time dim (dim 0) of the last audio block
                    last_time_max = llm_pos_ids_list[-1][0].max()
